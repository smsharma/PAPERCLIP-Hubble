...
def apply_rotary_embedding(
    ...
    if decode and qlen == 1 and rotary_index is not None:
        ...
        if q_position_offset is not None:
            qcos = jax.vmap(functools.partial(jax.lax.dynamic_slice_in_dim, slice_size=qlen, axis=0))(qcos, q_position_offset)
            qsin = jax.vmap(functools.partial(jax.lax.dynamic_slice_in_dim, slice_size=qlen, axis=0))(qsin, q_position_offset)
...
def generate_fixed_pos_embedding(features, length, min_timescale=1.0, max_timescale=10000.0):
    ...
    # from sin(256).
    sinusoid_inp = jnp.einsum("i , j -> i j", jnp.arange(length), rotational_frequency, precision=jax.lax.Precision.HIGHEST)
...
class CLIPVisionEmbeddings(nn.Module):
    ...
    def __call__(self, pixel_values):
        ...
        patch_embeds = nn.Conv(
            ...
            dtype=self.dtype,
            kernel_init=nn.with_logical_partitioning(nn.initializers.lecun_normal(), ("conv_height", "conv_width", "input_channels", "embed")),
        ...
        if self.position_embedding_type == "learnt":
            ...
            if self.position_embedding_factorized:
                position_embeds_height = self.param(
                    ...
                    nn.with_logical_partitioning(nn.initializers.normal(1 / np.sqrt(self.hidden_size)), (None, "height", "embed")),
                    (1, position_height, self.hidden_size),
                )
                position_embeds_width = self.param(
                    "position_embeds_width",
                    nn.with_logical_partitioning(nn.initializers.normal(1 / np.sqrt(self.hidden_size)), (None, "width", "embed")),
                ...
                if position_height != height:
                    ...
                    position_embeds_height = nn.with_logical_constraint(position_embeds_height, ("batch", "height", "embed"))
                if position_width != width:
                    # interpolate
                    position_embeds_width = jax.image.resize(position_embeds, (width,), method="linear")
                    position_embeds_width = nn.with_logical_constraint(position_embeds_width, ("batch", "width", "embed"))
                # make it 2d
                ...
                position_embeds = self.param(
                    "position_embeds",
                    nn.with_logical_partitioning(nn.initializers.normal(1 / np.sqrt(self.hidden_size)), (None, "vocab", "embed")),
...
class CLIPTextEmbeddings(nn.Module):
    ...
    def __call__(self, input_ids):
        ...
        embeddings = nn.Embed(
            self.vocab_size,
            embed_dim,
            embedding_init=nn.with_logical_partitioning(nn.initializers.normal(1 / np.sqrt(embed_dim)), ("vocab", "embed")),
            name="embeddings",
        )(input_ids.astype("i4"))
        embeddings = nn.with_logical_constraint(embeddings, ("batch", "length", "embed"))
        if self.position_embedding_type == "learnt":
...
class MultiHeadDotProductAttention(nn.Module):
    ...
    @nn.compact
    def __call__(self, inputs_q: Array, inputs_kv: Array, mask: Optional[Array] = None, deterministic: Optional[bool] = None):
...
class CLIPMLP(nn.Module):
    ...
    def __call__(self, inputs, deterministic: bool = False):
        ...
        with jax.profiler.TraceAnnotation("MLP_Block"):
            ...
            x = nn.Dropout(rate=self.mlp_dropout_rate, broadcast_dims=(-2,), name="mlp_dropout")(x, deterministic=deterministic)  # Broadcast along length.
            x = nn.with_logical_constraint(x, ("batch", "length", "mlp"))
...
class CLIPTextTransformer(nn.Module):
    ...
    def __call__(
        ...
        if not self.use_causal_mask:
            ...
            pooled_output = last_hidden_state[jnp.arange(last_hidden_state.shape[0]), _get_id_pos(input_ids, self.eos_token_id)]
...
class CLIPVisionTransformer(nn.Module):
    ...
    def __call__(
        ...
        if not (height == self.image_size and width == self.image_size and channels == 3):
            print(f"Warning: Input image size ({height}*{width}) doesn't match model ({self.image_size}*{self.image_size}).")
